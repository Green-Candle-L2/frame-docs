...
>
  <img src="https://i.imgur.com/6M3lec6.png" alt="Green Candle Image" />
</div>
...
Green Candle is an optimistic rollup which means it inherits many security properties from Ethereum.
...
Green Candle is an optimistic rollup which means it uses "fraud proofs" to determine whether withdrawals from the network are valid.
...
Green Candle is built using [Arbitrum's Nitro stack](https://docs.arbitrum.io/why-nitro).
...
Nitro is EVM compatible, so contracts that are written for mainnet can be deployed onto Green Candle without any modifications.
Applications requiring read or write access to Green Candle can employ the same processes used on mainnet, with the only difference being a different RPC and chain ID.

# Running a Green Candle Validator
...
For now, the only way to run a Green Candle testnet validator is to download and run a pre-built docker image.
In the future, we will open source our Nitro repository (with Green Candle modifications) and allow developers to build the image from source.
...
You will then need to download the Green Candle validator config that we've provided [here](https://github.com/frame-network/node-config).
